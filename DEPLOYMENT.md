...
| `Dockerfile` | 项目根目录 Dockerfile (Railway 直接读取) |
| `backend/Dockerfile` | backend 子目录 Dockerfile (Render/VPS 使用) |
...
4. 等待 Railway 读取项目根目录的 `railway.json` 和 `Dockerfile`
   - 系统会自动使用 `Dockerfile` builder
5. 点击 **Deploy Now**, 等待 3-5 分钟构建

> **如果遇到 "couldn't locate the dockerfile"**: 说明 Railway 没读到根目录 `Dockerfile`，删除服务重新部署即可。
